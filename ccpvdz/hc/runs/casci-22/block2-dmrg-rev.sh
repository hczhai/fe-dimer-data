...
export PYSCF_TMPDIR=/central/scratch/hczhai/hife/hc-def2-SVP-uks.12/runs/casci-22
...
if [ "$?" = "1" ] || [ "${SLURM_TASKS_PER_NODE}" = "" ] || [ "0" = "1" ]; then
    if [ "1" = "1" ]; then
        ...
        ln -s /central/scratch/hczhai/hife/hc-def2-SVP-uks.12/runs/casci-22/FCIDUMP ./FCIDUMP
        cp /central/scratch/hczhai/hife/hc-def2-SVP-uks.12/runs/casci-22/${SCPT}.conf ${SCPT}.conf.${TJ}
...
if [ "1" = "1" ]; then
    cp /central/scratch/hczhai/hife/hc-def2-SVP-uks.12/runs/casci-22/node0/1pdm.npy ${SCPT}.1pdm.${TJ}.npy
fi