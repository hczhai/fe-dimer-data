...
export PYSCF_TMPDIR=/central/scratch/hczhai/hife/hc-def2-SVP-uks.12/runs/select-18
...
if [ "$?" = "1" ] || [ "${SLURM_TASKS_PER_NODE}" = "" ] || [ "0" = "1" ]; then
    if [ "0" = "1" ]; then
        ...
        ln -s /central/scratch/hczhai/hife/hc-def2-SVP-uks.12/runs/select-18/FCIDUMP ./FCIDUMP
        cp /central/scratch/hczhai/hife/hc-def2-SVP-uks.12/runs/select-18/${SCPT}.conf ${SCPT}.conf.${TJ}
...
if [ "0" = "1" ]; then
    cp /central/scratch/hczhai/hife/hc-def2-SVP-uks.12/runs/select-18/node0/1pdm.npy ${SCPT}.1pdm.${TJ}.npy
fi