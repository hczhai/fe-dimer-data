...
export PYSCF_TMPDIR=/central/scratch/hczhai/hife/hc-def2-SVP-uks.13/runs/mf-1
...
if [ "$?" = "1" ] || [ "${SLURM_TASKS_PER_NODE}" = "" ] || [ "0" = "1" ]; then
    if [ "0" = "1" ]; then
        ...
        ln -s /central/scratch/hczhai/hife/hc-def2-SVP-uks.13/runs/mf-1/FCIDUMP ./FCIDUMP
        cp /central/scratch/hczhai/hife/hc-def2-SVP-uks.13/runs/mf-1/${SCPT}.conf ${SCPT}.conf.${TJ}
