...
export PYSCF_TMPDIR=/central/scratch/hczhai/hife/hs-def2-SVP-uks.11/runs/mf-21
...
if [ "$?" = "1" ] || [ "${SLURM_TASKS_PER_NODE}" = "" ] || [ "0" = "1" ]; then
    if [ "0" = "1" ]; then
        ...
        ln -s /central/scratch/hczhai/hife/hs-def2-SVP-uks.11/runs/mf-21/FCIDUMP ./FCIDUMP
        cp /central/scratch/hczhai/hife/hs-def2-SVP-uks.11/runs/mf-21/${SCPT}.conf ${SCPT}.conf.${TJ}
...
if [ "0" = "1" ]; then
    cp /central/scratch/hczhai/hife/hs-def2-SVP-uks.11/runs/mf-21/node0/1pdm.npy ${SCPT}.1pdm.${TJ}.npy
fi