...
#SBATCH --reservation=changroup
#SBATCH --job-name hs.8.hife
#SBATCH -o LOG.%j
#SBATCH -q long
#SBATCH --nodes=1
...
export PYSCF_TMPDIR=/central/scratch/hczhai/hife/hs-def2-SVP-uks.9/runs/cc-8
...
export SLURM_JOB_NAME=hs.8.hife
...
if [ "$?" = "1" ] || [ "${SLURM_TASKS_PER_NODE}" = "" ] || [ "0" = "1" ]; then
    if [ "0" = "1" ]; then
        ...
        ln -s /central/scratch/hczhai/hife/hs-def2-SVP-uks.9/runs/cc-8/FCIDUMP ./FCIDUMP
        cp /central/scratch/hczhai/hife/hs-def2-SVP-uks.9/runs/cc-8/${SCPT}.conf ${SCPT}.conf.${TJ}
...
if [ "0" = "1" ]; then
    cp /central/scratch/hczhai/hife/hs-def2-SVP-uks.9/runs/cc-8/node0/1pdm.npy ${SCPT}.1pdm.${TJ}.npy
fi
